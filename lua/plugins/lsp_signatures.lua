return {
    ...
    config = function()
        local opt = {
            ...
            floating_window_off_x = function() -- adjust float windows x position.
                local linenr = vim.api.nvim_win_get_cursor(0)[2] -- buf line number
                return linenr + 20
            end,
        ...
        vim.api.nvim_create_autocmd("LspAttach", {
            desc = "LspSignature Attach",
            group = vim.api.nvim_create_augroup("lsp_signature", {}),
            callback = function(a)
                -- local client = vim.lsp.get_client_by_id(a.data.client_id)
                require("lsp_signature").on_attach(opt, a.bufnr)
            end,
        })
    end,
}
